...
xdrawglyphfontspecs(const XftGlyphFontSpec *specs, Glyph base, int len, int x, int y, int dmode)
{
	...
	if (dmode & DRAW_FG) {
		...
		if (base.mode & ATTR_UNDERLINE) {
			...
			int linecolor;
			if ((base.ustyle & (UNDERCURL_COLOR_PALETTE | UNDERCURL_COLOR_RGB)) &&
				!(base.mode & ATTR_BLINK && win.mode & MODE_BLINK) &&
				!(base.mode & ATTR_INVISIBLE)
			) {
				// Special color for underline
				// Index
				if (base.ustyle & UNDERCURL_COLOR_PALETTE) {
					linecolor = dc.col[base.ustyle & 255].pixel;
				}
				// RGB
				else {
					XColor lcolor;
					lcolor.red = ((base.ustyle >> 16) & 255) * 257;
					lcolor.green = ((base.ustyle >> 8) & 255) * 257;
					lcolor.blue = (base.ustyle & 255) * 257;
			...
			if ((base.ustyle >> (24+UNDERCURL_COLOR_NBITS)) != UNDERCURL_TYPE_CURLY) {
				...
				if (undercurl_style == UNDERCURL_CURLY) {
					...
					free(arcs);
				} else if (undercurl_style == UNDERCURL_SPIKY) {
				...
				} else if (undercurl_style == UNDERCURL_CAPPED) {
